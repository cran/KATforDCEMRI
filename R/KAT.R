...
KAT <- function(file="concatenate.KAT.with.KAT.checkData.RData", results_file="my_results", method.optimization="L-BFGS-B", show.rt.fits=FALSE, param.for.avdt="Ktrans", range.map=1.5, cutoff.map=0.85, export.matlab=TRUE, export.RData=TRUE, verbose=FALSE, show.errors = FALSE, try.silent=TRUE, fracGTzero=0.75, AIF.shift="", Force.AIF.peak=FALSE, tlag.Tofts.on=FALSE, ...){
    ...
    unixid <- system("whoami",intern=TRUE)
...
##############################################################
#######SPECIFY THE ONE COMPARTMENT TOFTS MODEL################
##############################################################
...
########################################################################################
    roi.modelT <- function(p, t, dt, cp, zing=0) {
        if(zing==0){
            ...
            ##GF Set cp values at negative times to zero if AIF is arterial
            ## if(AIF.shift=="ARTERY")
            ## cp[tshift<0] <- 0
...
###############################################################
####SPECIFY THE OBJECTIVE FUNCTION FOR ESTIMATING ROI##########
###############################################################
    Obj_roi <- function(p, model, t, dt, cp, roi) {
        ## mean((model(p, t, dt, cp) - roi)^2)
...
##############################################################
####SPECIFY A NOMINAL STANDARD DEVIATION FOR EACH DATA POINT##
##############################################################
    map_cc_slice <- NULL
    map_cc_roi <- NULL
    aif <- NULL
    ...
    roi.median.fitted.5k <- NULL
    param.est.whole.roi.5k <- NULL
...
###IMPORT MATLAB FILE INTO R#######################
        if(file.format=="matlab"){
            ...
            map.Ktrans5k1 <- mat_data$mapKtrans5k1
            map.kep5k1 <- mat_data$mapkep5k1
            map.ve5k1 <- mat_data$mapve5k1
            map.Ktrans5k2 <- mat_data$mapKtrans5k2
            map.kep5k2 <- mat_data$mapkep5k2
            map.ve5k2 <- mat_data$mapve5k2
            map.vb5k <- mat_data$mapvb5k
            map.fitfailures5k <- mat_data$mapfitfailures5k
            map.AUC5k <- mat_data$mapAUC5k
            map.AUCMRT5k <- mat_data$mapAUCMRT5k
            ...
            param.est.median5k <- mat_data$paramestmedian5k
            ...
            roi.median.fitted.5k <- mat_data$roimedianfitted5k
            param.est.whole.roi.5k <- mat_data$paramestwholeroi5k
        ...
        if(file.format=="RData"){
            ...
            map.Ktrans5k1 <- dcemri.data$mapKtrans5k1
            map.kep5k1 <- dcemri.data$mapkep5k1
            map.ve5k1 <- dcemri.data$mapve5k1
            map.Ktrans5k2 <- dcemri.data$mapKtrans5k2
            map.kep5k2 <- dcemri.data$mapkep5k2
            map.ve5k2 <- dcemri.data$mapve5k2
            map.vb5k <- dcemri.data$mapvb5k
            map.fitfailures5k <- dcemri.data$mapfitfailures5k
            map.AUC5k <- dcemri.data$mapAUC5k
            map.AUCMRT5k <- dcemri.data$mapAUCMRT5k
            ...
            param.est.median5k <- dcemri.data$paramestmedian5k
            ...
            roi.median.fitted.5k <- dcemri.data$roimedianfitted5k
            param.est.whole.roi.5k <- dcemri.data$paramestwholeroi5k
                    ...
                    if(verbose==TRUE){
                        ...
                        if(class(fit_roi) != "try-error" & class(fit_roiT) != "try-error"){
                            if(fit_roi$convergence == 0 & fit_roiT$convergence == 0){
                                ...
                                ## AIC1 <- 0.5*(fit_roiT$value + log(2*pi)) + np_1/nt (saam ii aic)
                                ## AIC2 <- 0.5*(fit_roi$value + log(2*pi)) + np_2/nt (saam ii aic)
...
##############################################################
####CREATE A FOLDER BASED ON THE USERS UNIX ID################
##############################################################
            foldername <- unixid
...
####ZOOM IN ON TUMOR ROI#####
    ...
    if(file.original==FALSE){
        ...
        if(param.for.avdt == "AUC5k")
            param.median <- median(map.AUC5k[map.AUC5k>0], na.rm=TRUE)

        if(param.for.avdt == "AUCMRT5k")
            param.median <- median(map.AUCMRT5k[map.AUCMRT5k>0], na.rm=TRUE)
...
####TRUNCATE HIGH OUTLIER VALUES#####
        ...
        text(x_min/nx+(x_max/nx-x_min/nx)*0.84, y_min/ny+(y_max/ny-y_min/ny)*0.01, paste("KAT for DCEMRI v", KAT.version, ", Genentech EDPKPD", sep=""), col="darkgrey")
...
#########VOXEL LOCATOR####################
        ...
        while(inf == 1){
            ...
            if(xx < (x_min + 0.12*xdim) && yy > (y_max-0.02*ydim)){
                ...
                ## text=(x=x_min/nx, y=y_max/ny,  paste("R package version =", KAT.version))
                ...
                ## plot(0.5, 0.5, xaxt="n", yaxt="n", ylim=c(0,1), xlim=c(0,1), xlab="", ylab="", col="white", main="Function arguments for KAT()", cex.main=1.5)
                ## text(0.5, 0.99,  paste("R package version =", KAT.version), cex=1.5)
                ## text(0.5, 0.90,  paste("file =", strsplit(file, split="/")[[1]][length(strsplit(file, split="/")[[1]])]), cex=1)
                ## text(0.5, 0.81,  paste("slice =", slice), cex=1.5)
                ## text(0.5, 0.72,  paste("method.optimization =", method.optimization), cex=1.5)
                ## text(0.5, 0.63,  paste("range.map =", range.map), cex=1.5)
                ## text(0.5, 0.54,  paste("cutoff.map =", cutoff.map), cex=1.5)
                ## text(0.5, 0.45,  paste("lo =", lo), cex=1.5)
                ## frame()
